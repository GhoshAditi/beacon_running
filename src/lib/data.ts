...
export const data = {
  ...
  emails: {
    ...
    /** Email ids with revoked links (Firestore; used by dashboard warnings). */
    listRevokedIds: async (filters?: { companyId?: string }): Promise<string[]> => {
      if (filters?.companyId) {
        const emails = await data.emails.list({ companyId: filters.companyId });
        return emails.filter((e) => e.revoked === true).map((e) => e.id);
      }
      const q = query(emailsCollection, where('revoked', '==', true));
      const snapshot = await getDocs(q);
